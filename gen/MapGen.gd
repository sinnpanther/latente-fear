...
@export var room_recursion := 15
@export var min_room_size := 2
@export var max_room_size := 4
@export_multiline var custom_seed := "" :
    set(value):
        custom_seed = value
        seed(value.hash())
...
func generate():
    # Sécurité : attendre que la scène soit complètement prête
    await get_tree().process_frame

    generate_map()
    grid_map.visible = false
    await map_mesh.create_map()
...
func generate_runtime():
    generate_internal()

func get_spawn_position() -> Vector3:
    var spawn := room_positions[0]
    spawn.y = 1.0

    return spawn

func generate_internal():
    room_tiles.clear()
    room_positions.clear()

    if custom_seed != "":
        set("custom_seed", custom_seed)
    ...
    var delaunay := Array(Geometry2D.triangulate_delaunay(rpv2))
...
func make_room(rec: int):
    ...
    var avg_x := start_pos.x + (float(width) / 2)
    var avg_z := start_pos.z + (float(height) / 2)
    var pos := Vector3(avg_x, 0, avg_z)
